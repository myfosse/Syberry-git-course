...
create table hotels (id bigint not null, owner_id bigint, title varchar(255), location_id bigint, primary key (id)) engine=InnoDB;
...
create table saved_hotels (id bigint not null, hotel_id bigint, user_id bigint, primary key (id)) engine=InnoDB;
...
alter table saved_hotels add constraint FKaiph6iyn2pp1yyh6dyx3agofx foreign key (hotel_id) references hotels (id);
alter table saved_hotels add constraint FKryeliiac28y10r5gum5qipkhv foreign key (user_id) references users (id);
alter table hotels add constraint FKjlj2pyi45k16gjvlin6smjfjy foreign key (owner_id) references users (id);
